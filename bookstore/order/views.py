...
from django.db import transaction  # order_commit类需要用到
...
# 提交订单需要向两张表中添加数据
# s_order_info: 订单信息表，需要添加一条
# s_order_books: 订单商品表，订单中买了几个商品，需要添加几条
...
# 事务：原子性，一组sql操作，要么都成功，要么都不执行
# 开启事务： begin
# 事务回滚： rollback
# 事务提交： commit
# 设置保存点： savepoint 保存点_name
# 回滚到保存点： rollback 保存点_name
@transaction.atomic
def order_commit(request):
	...
	try:
		# 向订单信息表中添加一条数据
		order = OrderInfo.objects.create(order_id=order_id, passport_id=passport_id,
		...
		for id in books_ids:
			...
			# 创建一条商品记录
			OrderGoods.objects.create(order_id=order_id, books_id=id, count=count,
									  price=books.price)
			# 增加商品的销量，减少商品的库存
...
def check_pay(request):
	...
	if not order_id:
		return JsonResponse({'res': 1, 'errmsg': '订单不存在'})
	try:
		order = OrderInfo.objects.get(order_id=order_id, passport_id=passport_id,
									  pay_method=3)
	except OrderInfo.DoesNotExist:
		return JsonResponse({'res': 2, 'errmsg': '订单信息出错'})
	# 和支付宝进行交互
