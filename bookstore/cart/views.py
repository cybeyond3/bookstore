...
def cart_add(request):
	'''向购物车中添加数据'''
	data = json.loads(request.body.decode('utf-8'))  # 用json格式传递,也可以不用json

	# 1. 判断用户是否登录
	if not request.session.has_key('islogin'):  # 如果没有登录
		return JsonResponse({'res': 0, 'errmsg': '请先登录'})

	# 2. 接收数据
	# 注释掉的部分是不用json的时候接收数据的代码
	...
	# 3. 进行数据校验
	if not all([books_id, books_count]):  # 如果有数据为空
...
# 前端传过来的数据： 商品id books_id  更新数目 books_count
# post
# /cart/update/
